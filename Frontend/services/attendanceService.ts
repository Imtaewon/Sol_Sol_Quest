...
export const attendanceService = {
  ...
  getAttendanceData: async (year: number, month: number): Promise<ApiResponse<AttendanceData>> => {
    const response = await apiClient.get<ApiResponse<AttendanceData>>(`/api/v1/attendance/${year}/${month}`);
    return response.data;
  },

  // 출석 체크
  checkAttendance: async (data: AttendanceCheckRequest): Promise<ApiResponse> => {
    const response = await apiClient.post<ApiResponse>('/api/v1/attendance/check-in', data);
    return response.data;
  },
};
